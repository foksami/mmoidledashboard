  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      <div className="grid gap-2 pb-1.5 border-b border-[var(--color-border-subtle)] mb-1"
        ...
        <span className={`text-[9px] uppercase tracking-widest text-right transition-colors ${sort === "profit" ? "text-[var(--color-text-secondary)]" : "text-[var(--color-text-dim)]"}`}>
          Profit
        </span>
      </div>
      ...
      {sorted.map((row) => {
        ...
        const profitPos = (row.profitPerCraft ?? 0) > 0
              ...
              <div className="mt-1.5 pl-5 flex flex-col gap-1.5">
                ...
                <div className="flex items-center gap-2 text-[9px] text-[var(--color-text-dim)]">
                  {row.outputPrice != null && (
                    <span>Output: <span className="text-[var(--color-gold)]">{fmtNum(row.outputPrice)}g</span></span>
                  )}
